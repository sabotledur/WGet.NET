...
namespace WGetNET.HelperClasses
{
    ...
    internal static class ProcessOutputReader
    {
        ...
        public static List<WinGetPackage> ToPackageList(string[] output)
        {
            ...
            int availableVersionIndex = GetColumnStartIndex(output[labelLine], 4);
            int sourceIndex = GetColumnStartIndex(output[labelLine], 5);

            //Remove unneeded output Lines
            output = ArrayManager.RemoveRange(output, 0, labelLine + 2);

            return CreatePackageListFromOutput(output, idStartIndex, versionStartIndex, availableVersionIndex, sourceIndex);
        }
        ...
        /// </param>
        /// <param name="availableVersionIndex">
        ...
        private static List<WinGetPackage> CreatePackageListFromOutput(string[] output, int idStartIndex,
            int versionStartIndex, int availableVersionIndex, int sourceIndex)
        {
            List<WinGetPackage> resultList = new List<WinGetPackage>();

            foreach (string currentLine in output)
            {
                if (currentLine.Length >= sourceIndex)
                {
                    try
                    {
                        WinGetPackage winGetPackage = new WinGetPackage
                        {
                            PackageName = currentLine.Substring(0, idStartIndex).Trim(),
                            PackageId = currentLine.Substring(idStartIndex, versionStartIndex - idStartIndex).Trim(),
                            PackageVersion = currentLine.Substring(versionStartIndex, availableVersionIndex - versionStartIndex).Trim(),
                            PackageVersionAvailable = currentLine.Substring(availableVersionIndex, sourceIndex - availableVersionIndex).Trim(),
                            PackageSource = currentLine.Substring(sourceIndex).Trim()
                        };

                        resultList.Add(winGetPackage);
                    }
                    catch
                    {
        ...
        private static List<WinGetSource> CreateSourceListFromOutput(string[] output, int urlStartIndex)
        {
            ...
            for (int i = 0; i < output.Length; i++)
            {
                ...
                resultList.Add(
                    new WinGetSource()
                    {
                        SourceName = output[i].Substring(0, output[i].Length - urlStartIndex).Trim(),
                        SourceUrl = output[i].Substring(urlStartIndex).Trim(),
